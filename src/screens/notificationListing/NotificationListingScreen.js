...
const NotificationListingScreen = props => {
  ...
  const getNotificationListing = async () => {
    ...
    const response = await NetworkManager.fetchRequest(
      `${URL.END_POINT.notifications}${'?'}${'page='}${1}`,
  ...
  const getNotificationListingMore = async () => {
    if (isMorePage) {
      const response = await NetworkManager.fetchRequest(
        `${URL.END_POINT.notifications}${'?'}${'page='}${currentPage}`,
        URL.REQUEST_TYPE.getRequest,
      );
      setRefreshing(false);
      if (response.code == 200) {
        if (notificationsData?.length < response?.data?.total_count) {
          ...
          setIsMorePage(
            notificationsData?.length < response?.data?.total_count,
          );
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.keyboardAwareScroll}>
        ...
        <View
          style={{
            top: GlobalStyle.size.height / 3.8,
            position: 'absolute',
            bottom: 15,
