...
const GroupSelectionComponent = props => {
  const data = [
    ...
    {
      title: Strings.home.groupInvitations,
      count: props.invitationReceiveCounts,
    },
  ];

  const [selectedTitle, setSelectedTitle] = React.useState(
    data[props.passGroupSelectedIndex].title,
  );
      ...
      {data.map((item, index) => {
        return (
          <TouchableOpacity
            ...
            style={{
              backgroundColor:
                selectedTitle == item.title ? Colors.white : 'rgba(0,0,0,0.0)',
              width: '50%',
              borderTopLeftRadius:
                selectedTitle == Strings.home.allGroup ? 5 : 0,
              borderBottomLeftRadius:
                selectedTitle == Strings.home.allGroup ? 5 : 0,
              borderTopRightRadius:
                selectedTitle == Strings.home.groupInvitations ? 5 : 0,
            ...
            <Text
              style={styles.allGroupTxt}>{`${item.title}(${item.count})`}</Text>
          </TouchableOpacity>
        );
      })}
    </View>
