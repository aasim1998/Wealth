...
export default Utility = {
  ...
  //Amount validation
  _amountValidation: amount => {
    if (Constants.regex.amount.test(amount)) {
      return true;
    } else {
      return false;
    }
  ...
  _shortName: String => {
    return (
      String.slice(0, 1) +
      String.slice(String.indexOf(' ') + 1, String.indexOf(' ') + 2)
    );
  },
  ...
  _compareStringFormtatedDates: (date1, date2) => {
    return (
      date2.split('/').reverse().join('') > date1.split('/').reverse().join('')
    );
  },
  ...
  _dayDifferenceCalculatedBasisOfDateNPaymentPeriodTypeSelection: (
    date1,
    date2,
    recurenceType,
  ) => {
