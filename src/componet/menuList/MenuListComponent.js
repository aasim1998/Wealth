      ...
      <TouchableOpacity
        ...
        onPress={
          item.title === Strings.settings.rateApp
            ...
            : item.title === Strings.settings.delete
            ? props.onPressDeleteAccount
            : item.title === Strings.settings.logout
            ? props.onPressMenuList
            : () =>
                props.navigation.navigate(item.navigationKey, {
                  isFromScreen: item.title,
                })
        }
        style={[
          styles.onPress,
          {
            ...
            borderTopWidth:
              props.isFromMenuSreen || item.title === Strings.settings.logout
                ? 1
                : 0,
          },
        ]}>
            ...
            style={{
              ...
              fontFamily:
                item.title === Strings.settings.logout
                  ...
                  : props.isFromMenuSreen
                  ? Fonts.SFCompactDisplay.SemiBold
                  : Fonts.SFCompactDisplay.Light,
            }}>
            {item.title}
          </Text>
          {item.title === Strings.home.fundGroups && (
            <View style={styles.underLine}></View>
          )}
        </View>
        {item.title === Strings.settings.settings && (
          <View>
            <Image source={Assets.menu.forwardArrow} />
          </View>
        )}
        {item.title === Strings.settings.manageNotification && (
          <View>
            <Switch {...props} />
          </View>
        )}
