...
const ChangeMobileScreen = props => {
  ...
  const [countryId, setCountryId] = React.useState(
    props?.route?.params?.viewProfilePassData?.country_id,
  );
  const [country_Flag, setCountry_Flag] = React.useState(
    props?.route?.params?.viewProfilePassData?.flag,
  );
    ...
    if (isEntermobile) {
      if (mobile !== '' && mobile.length == 10) {
        ...
        if (response.code === 200) {
          if (response.message) {
            Utility._showToast(Strings.profile.otp_message);
          }
          setIsVerifyOtp(true);
        } else {
  ...
  const verfiyNowMethod = async () => {
    ...
    setIsLoaderVisible(true);
    const response = await NetworkManager.fetchRequest(
      URL.END_POINT.update_mobile,
      URL.REQUEST_TYPE.postRequest,
      updateMobileParameters,
    );
    setIsLoaderVisible(false);
    if (response.code === 200) {
      setIsSuccess(true);
        ...
        <KeyboardAwareScrollView
          ...
          {isSuccess ? (
            <SuccessScreenComponent
              ...
              successDescription={
                Strings.profile.yourMobileNoChangedSuccussfully
              }
              ...
              {isVerifyOtp ? (
                <VerifyOtpComponent
                  ...
                  verifyOtpDescription={
                    Strings.verifyOtp.pleaseEnterCodeToRegisteredMobile
                  }
                  ...
                  <ImageBackground
                    ...
                    <View style={styles.fieldContainer}>
                      <CustomTextInput
                        topPlaceholder={
                          isEntermobile
                            ? Strings.profile.newMobileNo
                            : Strings.signup.password
                        }
                        placeholder={
                          isEntermobile
                            ? Strings.profile.enterNewMobileNo
                            : Strings.signup.enterYourPassword
                        }
                        hideButton={!isEntermobile}
                        validationErrorMessageShow={
                          isPasswordShowErrorMessage
                            ? isPasswordShowErrorMessage
