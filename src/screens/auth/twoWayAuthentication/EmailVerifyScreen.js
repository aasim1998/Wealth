...
const EmailVerifyScreen = props => {
  ...
  const isFromForgotPassword =
    props?.route?.params?.forgotPassData?.isFromForgotPassword;
  const email =
    props?.route?.params?.forgotPassData?.emailForResetPassword ||
  ...
  const sendOtpMethod = async () => {
    ...
    const response = await NetworkManager.fetchRequest(
      URL.END_POINT.send_otp,
      URL.REQUEST_TYPE.postRequest,
      emailSendOtpParameters,
    );
      ...
      if (response.code === 200) {
        if (isFromForgotPassword) {
          props.navigation.replace('ResetPassword', {
            data: {
              otp: enteredOtp,
              token: props?.route?.params?.forgotPassData?.token,
            },
          });
        } else {
  ...
  useEffect(() => {
    if (
      props?.route?.params?.signupPassData?.isFromSignup === true ||
      props?.route?.params?.loginPassData?.isFromLogin === true
    ) {
      sendOtpMethod();
    }
  },[]);
        ...
        <KeyboardAwareScrollView
          ...
          <VerifyOtpComponent
            ...
            verifyOtpDescription={
              Strings.verifyOtp.pleaseEnterCodeToRegisteredEmail
            }
          ...
          {isSuccessModalVisible && (
            <SuccessModal
              ...
              succesModelDesciption={
                Strings.sussessModel.yourAccountHasbeenCreated
              }
