...
const Header = props => {
  return (
    <ImageBackground
      style={[
        styles.container,
        { backgroundColor: props.headerImg ? Colors.white : 'rgba(0,0,0,0.0)' },
      ]}
      ...
      <View style={styles.leftContainer}>
        <TouchableOpacity
          ...
          <Image source={props.onPressLeftIcon} />
        </TouchableOpacity>
        {props.isChat && (
          <>
            {props.memberImage != null ? (
              ...
              <View style={styles.shortNameContainer}>
                <Text style={[styles.shortsName]}>
                  {props.shortName != null ? props.shortName.toUpperCase() : props.shortName}
                </Text>
              </View>
            )}
          </>
        ...
        <View style={styles.headerTitleContainer}>
          {!props.isForNotifications && (
            <Text style={styles.headerTitleTxt}>
              {props.headerTitle.toUpperCase()}
            </Text>
          )}
          {props.isForNotifications && (
            <Text
              ...
              }>{`${props.headerTitle.toUpperCase()} (${props.notificationsCount
                })`}</Text>
          )}
        </View>
      </View>
      <View
        style={[
          ...
          {
            flex: props.isFilter ? 3 : 1,
            paddingRight: props.home ? GlobalStyle.size.width / 13 : 0,
          },
        ]}>
        {/* walleticon */}

        {!props.home || props.isFilter ? null : (
          <>
            <TouchableOpacity
              style={styles.wallet}
              onPress={props.onPressWalletIcon}>
              <Image
                source={props.walletIcon}
                style={{
            ...
            <>
              ...
              <View style={styles.resetTitleContainer}>
                <Text style={styles.resetTitle}>
                  {Strings.home.filter.reset}
                </Text>
              </View>
            </>
          ) : (
            <Image source={props.onPressRightIcon} />
          )}
        </TouchableOpacity>
      </View>
    </ImageBackground>
